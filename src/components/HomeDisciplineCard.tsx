...
export default function HomeDisciplineCard({ discipline, index }: { discipline: Discipline; index: number }) {
  ...
  return (
    <Link
      ...
      className={`block min-h-56 border-b border-r border-white/10 bg-[#0B0B0D] p-6 hover:bg-[#151515] hover:-translate-y-0.5 motion-reduce:hover:translate-y-0 sm:p-8 ${revealClass} ${revealState(visible)}`}
    >
      <span className="text-xs text-[#FF7A33]">[{discipline.id}]</span>
      <h3 className="mt-10 text-2xl font-semibold tracking-[-0.04em] text-white">{discipline.title}</h3>
      <p className="mt-3 max-w-sm text-sm leading-6 text-[#90939A]">{discipline.description}</p>
